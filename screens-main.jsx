...
function HomeScreen({ places: propPlaces, onSelectPlace, budget, onBudgetChange, budgetMode, savedIds, onToggleSave }) {
  ...
  const allPlaces = propPlaces || PLACES;
  ...
  const filteredPlaces = React.useMemo(() => {
    ...
    return result;
  }, [allPlaces, budget, searchQuery, activeFilters, sortBy]);
  ...
  return React.createElement('div', {
    ...
    React.createElement('div', {
      ...
      React.createElement('div', { style: { marginTop: 8, display: 'flex', alignItems: 'center', gap: 8 } },
        React.createElement('button', {
          ...
          React.createElement('svg', { width: 16, height: 16, viewBox: '0 0 24 24', fill: t.textSecondary },
            viewMode === 'map'
    ...
    viewMode === 'map'
      ...
      // List view
      : React.createElement('div', { style: { flex: 1, overflow: 'auto', padding: '8px 16px 16px' } },
          React.createElement('div', {
...
function PlaceDetailScreen({ place, onBack, savedIds, onToggleSave }) {
  const t = useTheme();
  const [mealItems, setMealItems] = React.useState([]);
  const [activeSection, setActiveSection] = React.useState(0);
  const [menu, setMenu] = React.useState(place.menu || []);
  const [menuLoading, setMenuLoading] = React.useState(!place.menu || place.menu.length === 0);

  const isSaved = savedIds && savedIds.has(place.id);

  // Lazy-load menu from DB if not already on place object
  React.useEffect(() => {
    if (!place.menu || place.menu.length === 0) {
      fetchMenuForPlace(place.id).then(m => {
        setMenu(m);
  ...
  return React.createElement('div', {
    style: { display: 'flex', flexDirection: 'column', height: '100%', background: t.bg, overflow: 'auto' },
  },
    // Hero
    React.createElement('div', {
      style: {
      ...
      React.createElement('span', { style: { fontSize: 64, opacity: 0.3 } }, foodIcon),
      // Back
      React.createElement('button', {
        onClick: onBack,
        style: {
          position: 'absolute', top: 12, left: 12, width: 36, height: 36,
          borderRadius: 12, background: 'rgba(255,255,255,0.85)', border: 'none',
          cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center',
      ...
      React.createElement('div', {
        ...
        React.createElement('button', {
          ...
          style: {
            ...
            background: isSaved ? t.primary : 'rgba(255,255,255,0.85)', border: 'none',
            cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center',
            backdropFilter: 'blur(8px)', transition: 'all 0.2s',
          },
        },
          React.createElement('svg', { width: 16, height: 16, viewBox: '0 0 24 24', fill: isSaved ? '#fff' : '#1a1a1a' },
    ...
    React.createElement('div', {
      style: {
        ...
        marginTop: -20, borderTopLeftRadius: 20, borderTopRightRadius: 20, position: 'relative',
      },
    },
      React.createElement('h1', {
        style: { fontSize: 22, fontWeight: 800, color: t.text, fontFamily: t.font.heading, margin: 0, letterSpacing: '-0.5px' },
      }, place.name),
      React.createElement('div', { style: { fontSize: 13, color: t.textSecondary, fontFamily: t.font.body, marginTop: 4 } },
        place.location),
      React.createElement('div', { style: { display: 'flex', alignItems: 'center', gap: 12, marginTop: 10, flexWrap: 'wrap' } },
        React.createElement(StarRating, { rating: place.rating, size: 14 }),
        React.createElement('span', { style: { fontSize: 12, color: t.textTertiary, fontFamily: t.font.body } },
          `${(place.reviews || place.review_count || 0).toLocaleString()} reviews`),
      ...
      React.createElement('div', {
        style: {
          marginTop: 16, padding: '14px 16px', background: t.surfaceAlt,
          borderRadius: 14, display: 'flex', justifyContent: 'space-between', alignItems: 'center',
        },
      },
        React.createElement('div', null,
          React.createElement('div', { style: { fontSize: 12, color: t.textSecondary, fontFamily: t.font.body } }, 'Typical meal cost'),
          React.createElement('div', {
            style: { fontSize: 26, fontWeight: 800, color: t.primary, fontFamily: t.font.heading, letterSpacing: '-1px' },
          }, `$${(place.typicalMeal || 0).toFixed(2)}`)
        ),
        React.createElement('div', { style: { textAlign: 'right' } },
          React.createElement('div', { style: { fontSize: 11, color: t.textTertiary, fontFamily: t.font.body } }, 'Price range'),
          React.createElement('div', { style: { fontSize: 14, fontWeight: 700, color: t.text, fontFamily: t.font.heading } },
            `$${(place.priceRange || [0, 0])[0].toFixed(2)} – $${(place.priceRange || [0, 0])[1].toFixed(2)}`)
        )
      ),
      React.createElement('button', {
        style: {
          ...
          fontSize: 15, fontWeight: 700, fontFamily: t.font.heading, cursor: 'pointer',
    ...
    React.createElement('div', { style: { padding: '16px', background: t.bg } },
      React.createElement('div', {
        ...
        React.createElement('h2', {
          style: { fontSize: 18, fontWeight: 800, color: t.text, fontFamily: t.font.heading, margin: 0, letterSpacing: '-0.3px' },
        }, 'Menu & Prices'),
        React.createElement('span', { style: { fontSize: 11, color: t.textTertiary, fontFamily: t.font.body } }, 'All prices in SGD')
      ),
      menuLoading
        ? React.createElement('div', { style: { textAlign: 'center', padding: '24px', color: t.textTertiary, fontFamily: t.font.body } }, 'Loading menu...')
        : React.createElement('div', null,
            React.createElement('div', {
    ...
    mealItems.length > 0 && React.createElement('div', {
      style: {
        position: 'sticky', bottom: 0, padding: '12px 16px', background: t.surface,
        borderTop: `1px solid ${t.border}`, boxShadow: `0 -4px 12px ${t.shadow}`,
      },
    },
      React.createElement('div', { style: { display: 'flex', justifyContent: 'space-between', alignItems: 'center' } },
        React.createElement('div', null,
          React.createElement('div', { style: { fontSize: 12, color: t.textSecondary, fontFamily: t.font.body } },
        ...
        React.createElement('div', { style: { fontSize: 12, color: t.textTertiary, fontFamily: t.font.body, textAlign: 'right' } },
          'Estimated total')
      )
    ),
    ...
    React.createElement('div', { style: { padding: '0 16px 24px', background: t.bg } },
      React.createElement('h2', {
        style: { fontSize: 16, fontWeight: 700, color: t.text, fontFamily: t.font.heading, margin: '0 0 10px', letterSpacing: '-0.3px' },
      }, 'Price Accuracy'),
      React.createElement('div', {
        style: { background: t.surface, borderRadius: 14, padding: '14px', border: `1px solid ${t.border}` },
      },
        React.createElement('div', { style: { display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8 } },
          React.createElement('div', {
            style: { width: 40, height: 40, borderRadius: 10, background: `${t.verified}15`, display: 'flex', alignItems: 'center', justifyContent: 'center' },
          },
          ...
          React.createElement('div', null,
            React.createElement('div', { style: { fontSize: 14, fontWeight: 700, color: t.text, fontFamily: t.font.heading } }, '94% accurate'),
            React.createElement('div', { style: { fontSize: 11, color: t.textTertiary, fontFamily: t.font.body } }, 'Based on 47 community reports')
          )
        ),
        React.createElement('div', { style: { fontSize: 12, color: t.textSecondary, fontFamily: t.font.body, lineHeight: 1.5 } },
          `Prices last confirmed by 12 users this week. Most recent check: ${place.verifiedDate || 'recently'}.`)
      )
    )
  );
}
